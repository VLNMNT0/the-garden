local DarkTransitionPlus, super = Class(Object)

function DarkTransitionPlus:init(final_y, options)
    super.init(self)
    ...
    for _, character in ipairs(Game.party) do
        local party_char = Game.world:getPartyCharacterInParty(character)
    
        table.insert(default_characters, party_char)
    end
...
function DarkTransitionPlus:onAddToStage(stage)
    for _, music in ipairs(Music.getPlaying()) do
        music:fade(0, 20 / 30)
    end
end

function DarkTransitionPlus:drawDoor(x, y, xscale, yscale, rot, color)
...
function DarkTransitionPlus:update()
    super.update(self)
...
function DarkTransitionPlus:draw()

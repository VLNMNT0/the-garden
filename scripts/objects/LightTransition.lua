...
function LightTransition:afterEndCallback()
...
function LightTransition.cutscene(cutscene, self, player)
	...
	if kris and susie or kris and noelle then
	...
	cutscene:walkTo(kris, event.x + ((self.kris_only and 0) or distance), event.y + event.height + 40, wait, 'down', true)

	if susie and not self.kris_only then
		cutscene:walkTo(susie, event.x - distance, event.y + event.height+ 40, wait, 'down', true)
	elseif noelle and not self.kris_only then
		cutscene:walkTo(noelle, event.x - distance, event.y + event.height+ 40, wait, 'down', true)
	end
	...
	--fakeKris:setOrigin(0.5, 0.5)
	fakeKris:setScale(2, 2)
	fakeKris:play(1 / 15, true)

	fakeKris.layer = settings.fake_layer

	Game.world:addChild(fakeKris)
	...
	if susie and not self.kris_only then
		...
		--fakeKris:setOrigin(0.5, 0.5)
		fakeSusie:setScale(2, 2)
		fakeSusie:play(1 / 15, true)

		fakeSusie.layer = settings.fake_layer

		Game.world:addChild(fakeSusie)
        elseif noelle and not self.kris_only then
		noelle.visible = false
		fakeNoelle = Sprite("party/noelle/dark_transition/dark", noelle.x - 29, noelle.y - 86)
		fakeNoelle.physics.speed_y = settings.fake_speed_y
		fakeNoelle.physics.gravity = settings.fake_gravity
		fakeNoelle.physics.speed_y = settings.fake_speed_y
		fakeNoelle.physics.gravity = settings.fake_gravity

		fakeNoelle:setScale(2, 2)
		fakeNoelle:play(1 / 15, true)

		fakeNoelle.layer = settings.fake_layer

		Game.world:addChild(fakeNoelle)
	end
	--]]

	self:beforeFadeCallback()
	cutscene:wait(cutscene:fadeOut(settings.fadeOut_speed, { color = { 1, 1, 1 } }))
	cutscene:loadMap(self.map)
	self:afterFadeCallback()

	fakeKris:remove()
	if fakeSusie then fakeSusie:remove() end
	if fakeNoelle then fakeNoelle:remove() end
	kris.visible = true
	if susie then
		susie.visible = true
	elseif noelle then
...
function LightTransition:update()
	...
	if (self.particletimer >= 2) then
		...
		local particle = LightTransitionParticle(math.random((ox - (self.width * .75)), (ox + (self.width * .75))),
			(oy - math.random(self.height + 240)))
...
function LightTransition:drawLight(moreAlpha)
	...
	for i = 1, 3 do
		...
		love.graphics.setColor(1, 1, 1, alpha + moreAlpha)
		love.graphics.draw(self.pillar, 0, -240, 0, ((self.pillarscale - (0.3 * i)) + (math.sin(sinoff) * 0.1)), 1, ox,
			-self.height)

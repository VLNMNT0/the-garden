return {
	...
	gateev = cutscene:getEvent("noellegate")

	if Game.inventory:getItemByID("holiday_gate_key") then
	    local gatechoicer = cutscene:choicer({"Unlock", "Do not"})
	    if gatechoicer == 1 then
                cutscene:wait(cutscene:walkTo(Game.world.player, "gate_unlock", 0.5, "left", true))
	        gateev:open()
	    else
    ...
    flush = function(cutscene, event)
        cutscene:text("* (It's a toilet.)\n[wait:10]* (Flush it?)")
	local flushchoice = cutscene:choicer({"Yes", "No"})
	if flushchoice == 1 then
            cutscene:wait(cutscene:playSound("toilet"))

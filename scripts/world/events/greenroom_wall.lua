...
function GreenRoomWall:init(data)
	...
	self.wallstars = Assets.getTexture("world/castle_town/tv/green_room_bg_fx_wallrow")
...
function GreenRoomWall:onLoad()
	...
	if self.spawn_vines == true then
		self.vines = Sprite("world/castle_town/tv/green_room_vines", self.x, self.y)
	...
	if self.spawn_shines == true then
		...
		for i = 0, max_amount do
			...
			local shine = Sprite("world/castle_town/tv/shine_white", self.x + (i * 55), self.y + y_offset)
			shine:setScale(2,2)
		...
		for i = 0, max_amount do
			...
			local shine = Sprite("world/castle_town/tv/shine_white", self.x + 5 + (i * 75) + x_offset, self.y + 5 + y_offset)
			shine:setScale(2,2)
			shine.color = {232/255, 1, 200/255}
			shine:play(1/2.4)
			shine:setFrame(starting_index)
